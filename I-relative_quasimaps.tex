...
Before getting to a Gathmann-type recursion formula for relative quasimaps, I shall discuss virtual fundamental classes. First I show that there is a meaningful virtual pullback along $\Phi_{|Y|}$, following Appendix B of the first version of \cite{BN}.
...
\noindent where $r_X = \operatorname{rk} \Pic X$ (implicitly we have chosen a basis for $\Pic X$) and the first arrow can be described as the composition of the derivative of the torus ($\Gm^{r_X}$)-action on $\Aaff^{\Sigma_X(1)}$ with the diagonal matrix $(u_\tau)_{\tau\in\Sigma_X(1)}$.
...
\noindent where one needs to use the isomorphisms $\varphi_{\mathbf b-\mathbf a}$ as above in order to make sense of the sum as a section of one fixed line bundle. On the other hand writing $\OO_X(1)$ in the preferred basis of $\Pic(X)$ gives a vector $v\in\ZZ^{r_X}$.

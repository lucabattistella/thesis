...
\subsection{Definitions} Let $N$ be a lattice with dual $M=\Hom_{\ZZ}(N,\ZZ)$\index{Toric geometry!$N,M=\Hom_{\ZZ}(N,\ZZ)$ dual lattices}, $\Sigma\subseteq N_{\RR}$\index{Toric geometry!$\Sigma\subseteq N_{\RR},X_\Sigma$ strictly convex rational polyhedral lattice and associated toric variety} a rational polyhedral fan with associated toric variety $X_{\Sigma}$, with primitive generators $v_{\rho}$ of the rays $\rho\in\Sigma(1)$\index{Toric geometry!$\rho\in\Sigma(1)$ ray i.e. toric divisor}. I assume throughout this text that $X=X_{\Sigma}$ is smooth and projective (equivalently every cone is spanned by a $\Z$-basis and the support of the fan is the whole of $N_{\RR}$).
...
\begin{lem}
 There is a birational \ilemph{collapsing} morphism $\chi\colon \M{0}{n}{\PP^N}{d}\to \Q{0}{n}{\PP^N}{d}$ \index{Quasimaps!$\chi\colon\M{0}{n}{\PP^N}{d}\to\Q{0}{n}{\PP^N}{d}$ collapsing morphism}.
\end{lem}
...
\begin{rmk} We have proved in \cite[Lemma B.5]{BN} that if $f$ is finite onto its image (i.e. it does not contract any effective curve class in $X$) and $\Q{g}{n}{Y}{f_*\beta}$ is unobstructed, then $Q(f)$ admits a perfect obstruction theory compatible with the ones concerned, hence $Q(f)^![\Q{g}{n}{Y}{f_*\beta}]=\virt{\Q{g}{n}{X}{\beta}}$. I review this in a special case in Lemma \ref{Exists relative POT} below. \end{rmk}

\begin{rmk}
I. Ciocan-Fontanine has kindly pointed out that, contrary to the case of stable maps, $Q(f)$ might not be a closed embedding, even if $f$ is. Consider the Segre embedding:
\begin{align*}
...
Finally define the $\epsilon$-\emph{quasimap quantum product}\index{Quasimaps!$\circ_\epsilon$ the $\epsilon$-quasimap quantum product} on generators by:
...
\begin{definition}
 ...
 \emph{Batyrev's quantum ring}\index{Quasimaps!$QH^*_\omega(X_\Sigma,\mathbb C)$ Batyrev's quantum ring of $X_\Sigma$} is a $q$-deformation of ordinary cohomology, defined as \[Q\!H^*_\omega(X_\Sigma,\mathbb C):=\mathbb C[x_\rho]_{\rho\in\Sigma(1)}/(P(\Sigma)+SR_\omega(\Sigma)),\] where \[SR_\omega(\Sigma)=\langle x_{\rho_1}\cdots x_{\rho_k}-q^{\beta(\mathcal P)}x_{\rho'_1}^{c_1}\cdots x_{\rho'_h}^{c_h} : \mathcal P\text{ is a primitive collection}\rangle.\]
\end{definition}
...
Let me denote by $\{\sigma_i\}_{i\in\Sigma^\text{max}}$ \index{Toric geometry!$\{\sigma_i\}_{i\in\Sigma^\text{max}},\{\tau_{i,j}\}_{i,j\in\Sigma^\text{max}}$ torus-fixed points and $1$-dimensional orbits} the $T$-fixed points on $X$ (corresponding to maximal cones of $\Sigma$) and by $\{\tau_{i,j}\}_{i,j\in\Sigma^\text{max}}$ the $1$-dimensional orbits (corresponding to facets of the maximal cones; if it exists, $\tau_{i,j}$ connects $\sigma_i$ and $\sigma_j$).
Write $\sigma_i\diamond\sigma_j$\index{Toric geometry!$\diamond$ adjacency between maximal cones} if $\sigma_i$ and $\sigma_j$ are two adjacent maximal cones; since $X$ is smooth, $\{v_\rho\}_{\rho\prec\tau_{i,j}}\cup\{v_d\}$ is a $\ZZ$-basis of $N$ (where $v_d$ is the only ray in $\sigma_i$, but not in $\tau_{i,j}$), so we can find the dual basis $\{m_1,\ldots,m_d\}$ of $M$. Define\index{Toric geometry!$\lambda^{\sigma_i}_{\sigma_j}, \lambda^{\sigma}_{\text{tot}}$ torus weights}: \[\lambda^{\sigma_i}_{\sigma_j}=\sum_{\rho\in\Sigma(1)}\langle m_d,v_\rho\rangle\lambda_\rho \quad \text{and} \quad \lambda^{\sigma}_{\text{tot}}=\prod_{\sigma'\in\Sigma^{\text{max}}\colon\sigma'\diamond\sigma}\lambda^\sigma_{\sigma'}\] Notice that there is a correspondence between rays of $\sigma$ and adjacent maximal cones, via the complementary facet, resp. the intersection; I will denote by $\sigma(\rho)$\index{Toric geometry! $\sigma(\rho)$ correspondence between rays and adjacent maximal cones} the adjacent maximal cone corresponding to $\rho\prec\sigma$. Compare with \cite[\S\S 6.4 and 7.3]{HolgerSpielberg}.
...
One more convention: for a flag $f=(v,e)$, if $\vv(v)=\sigma_i$ and $\varepsilon(e)=\tau_{i,j}$, denote by $\lambda_f:=\frac{1}{\delta(e)}\lambda^{\sigma_i}_{\sigma_j}$\index{Toric geometry!$\lambda_f$ torus weight associated to a flag $f$}.
...
More generally, if we assume that $Y$ is a very ample smooth divisor in the smooth variety $X$, we may use the complete linear system associated to $Y$ in order to embed $\phi_{\lvert Y\rvert}\colon X\hookrightarrow\PP^N$, and find a hyperplane $H\subseteq \PP^N$ such that $H\cap X= Y$. Set $d=\phi_{\lvert Y\rvert,*}(\beta)$ for a curve class $\beta\in H_2^+(X)$. Define $\M{0}{\alpha}{X|Y}{\beta}$\index{Quasimaps!$\M{0}{\alpha}{XrelY}{\beta}$ Gathmann's space of relative stable maps} by the following cartesian diagram:
\bcd
...
\end{thm}
where $\mathcal{D}_{\alpha,k}(X,\beta)$\index{Quasimaps!$\mathcal{D}_{\alpha,k}(X,\beta), D(XrelY,A,B,M)$ Gathmann's comb loci} is a sum over all
\begin{itemize}
...
\bcd
\mathcal D(X|Y,A,B,M)\ar[r]\ar[d]\ar[dr,phantom,"\Box"] & \M{0}{\lvert\alpha^{(0)}\rvert+r}{Y}{\beta_0}\times\prod_{i=1}^r
...
\begin{definition}
 ...
 More generally, if $X$ is a smooth toric variety and $Y\subseteq X$ a smooth and very ample divisor, the closed embedding $\phi_{\lvert Y\rvert}\colon X\hookrightarrow \PP^N$ induces $\Phi_{\lvert Y\rvert}\colon \Q{0}{n}{X}{\beta}\to \Q{0}{n}{\PP^N}{d}$ by functoriality (\S \ref{sec:functoriality}), and we may define the relative space as the fiber product\index{Quasimaps!$\Q{0}{\alpha}{XrelY}{\beta}$ space of relative quasimaps}:
\bcd
...
\bcd
k^* \EE_{\om{Q}(\PP^N)/\MM} \ar[r] \ar[d] & \EE_{\om{Q}(X)/\MM} \ar[r] \ar[d] & \EE_k  \ar[r,"{[1]}"] \ar[d] & \, \\
k^*\LL_{\om{Q}(\PP^N)/\MM} \ar[r] & \LL_{\om{Q}(X)/\MM} \ar[r] & \LL_k \ar[r,"{[1]}"] & \,
\ecd
...
\begin{rmk}[Quasimaps to $Y$] Under our assumptions $Y$ needs not be toric. The moduli space of (absolute) quasimaps to $Y$ can be defined via the cartesian diagram:
\bcd
\Q{0}{n}{Y}{\beta}\ar[d, hook] \ar[r]\ar[rd,phantom,"\square"] & \Q{0}{n}{H}{d} \ar[d, hook] \\
\Q{0}{n}{X}{\beta}\ar[r, "\Phi_{|Y|}"] & \Q{0}{n}{\PP^N}{d}
\ecd
...
\end{prop}
\noindent where $\mathcal{D}^Q_{\alpha,k}(X,\beta)$\index{Quasimaps!$\mathcal{D}^Q_{\alpha,k}(X,\beta), \mathcal D^{\mathcal{Q}}(XrelY,A,B,M)$ quasimap comb loci} is a sum of ``comb loci'', running over all splittings $A,B,M$ of the markings, curve class, and multiplicities at the internal-to-external nodes, satisfying
...
\begin{proof}
 ...
 \end{equation}
since collapsing does not affect the universal curve in a neighbourhood of the marking $x_k$. On the other hand, if a comb locus $\mathcal D(\PP^N|H,A,B,M)$ contains a rational tail (necessarily external), then the restriction of $\chi$ to $\mathcal D(\PP^N|H,A,B,M)$ is not finite over its image, because $\dim(\M{0}{(m^{(i)})}{\PP^N|H}{d})>\dim H$, so the corresponding term of Gathmann's formula vanishes under $\chi_*$. To be more precise, there is a cartesian diagram
\bcd
\mathcal D(\PP^N|H,A,B,M)\ar[r]\ar[d,"\chi"]\ar[dr,phantom,"\Box"] & \mathcal E(\PP^N|H,A,B,M)\ar[d,"\prod\chi"] \\
\mathcal {D^Q}(\PP^N|H,A,B,M)\ar[r]\ar[d]\ar[dr,phantom,"\Box"] & \mathcal {E^Q}(\PP^N|H,A,B,M)\ar[d,"\ev"] \\
H^r\ar[r,"\Delta_{H^r}"] & H^r\times H^r
\ecd
(where $\mathcal E$ is the usual - not -fiber - product of the relevant (quasi)map spaces, see below) and by commutativity of proper pushforward with Gysin maps we can argue as above at the level of $\mathcal E$, where we are actually pushing forward the fundamental class.
...
Let me introduce some more useful notation at this point: consider the product (\emph{not} the fiber product over $Y^r$)\index{Quasimaps!$\mathcal E^{\mathcal{Q}}(XrelY,A,B,M)$ ordinary product of quasimaps to $Y$ and relative quasimaps to $(XrelY)$}
...
\Q{0}{n}{X}{\beta} \ar[r,"\Phi_{\lvert Y\rvert}"] & \Q{0}{n}{\PP^N}{d}
\ecd

We shall also denote by $\mathcal{D}^{\mathcal{Q}}(X,A,B)$ and $\mathcal{E}^{\mathcal{Q}}(X,A,B)$ the fiber product (resp. ordinary product)\index{Quasimaps!$\mathcal{D}^{\mathcal{Q}}(X,A,B), \mathcal{E}^{\mathcal{Q}}(X,A,B)$ fiber (resp. ordinary) product of quasimaps to $X$} above where every occurrence of relative quasimaps to $(X|Y)$ and absolute quasimaps to $Y$ have been replaced by absolute quasimaps to $X$.
...
\bcd
\mathcal D(X)\ar[r]\ar[d,"\psi_{{}_X}"]\ar[dr,phantom,"\Box"] & \mathcal D(\PP^N)\ar[r]\ar[d,"\psi_{\PP^N}"]\ar[dr,phantom,"\Box"] & \MM_{A,B}^{\operatorname{wt}}\ar[d,"\psi"] \\
\om{Q}(X)\ar[r,"\Phi_{\lvert Y\rvert}"] & \om{Q}(\PP^N)\ar[r] & \MM_{0,n,\beta}^{\operatorname{wt}}
\ecd
...
It can be proved that invariants \emph{of $Y$} comprising \emph{only one} unrestricted insertion vanish: assume we are interested in $\langle\tau_{a_1}(\delta_1),\ldots,\tau_{a_n}(\delta_n)\rangle^{Y,0+}_{0,n,\beta}$ with only $\delta_1\in i^*H^*(X)^\perp$. Recall that $i_*\virt{\Q{0}{n}{Y}{\beta}}=c_{\text{top}}(\pi_*f^*\OO_X(Y))\cap \virt{\Q{0}{n}{X}{i_*\beta}}$ follows from pulling back the analogous statement for $(\PP^N|H)$, and we may write $c_{\text{top}}(\pi_*f^*\OO_X(Y))=\ev_1^*\OO_X(Y)c_{\text{top}}(E^{Y\cdot\beta})$ for a certain vector bundle $E^{Y\cdot\beta}$. Consider the following factorisation:
\bcd
...
Recall that we have fixed a homogeneous basis $\eta_0, \ldots, \eta_l$ for $\HH^*(X) = \HH^*(X,\QQ)$ and let $\eta^0, \ldots, \eta^l$ denote the dual basis with respect to the Poincar\'e pairing. Without loss of generality we may suppose that $\eta^0=\mathbbm 1_X$ and $\eta^1=[Y]$. We get an induced system of generators $\rho_1=i^*\eta_1, \ldots, \rho_l = i^* \eta_l$ for $i^*\HH^*(X)$ (some are possibly zero when the cohomological degree is bigger than $\dim X-1$). Notice that $\rho_0 = i^* \eta_0 = i^* [\pt_X] = 0$, $\rho_1 = i^* \eta_1 = [\pt_Y]$.  After possibly extracting a linearly independent subset of $\{\rho_j\}$ that spans $i^*\HH^*(X)$ and renumbering, we may extend it to a basis $\rho_1, \ldots, \rho_k$ for $\HH^*(Y)$ by adding a basis $\rho_{\tilde l+1},\ldots,\rho_{k}$ of $i^*\HH^*(X)^\perp$. Let $\rho^1, \ldots, \rho^k$ denote the dual basis; notice that $\rho^i$ is \emph{not} equal to $i^* \eta^i$, and $\rho^1 = \mathbbm{1}_Y$.
...
where by convention $S_0^X(z,0)= \mathbbm 1_X$;\index{Quasimaps!$S_0^X(z,q)$ $2$-pointed $1$-descendant generating function of quasimap invariants} this function takes values in $\HH^*(X,\Lambda)$ and the notation is reminiscent of $I(q,z)=S^{0+}(q,0,z)(\mathds 1)$.

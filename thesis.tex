...
\end{figure}
thic curve is not $3$-stable. We could contract the maximal unpointed subcurve of genus one, but it is not balanced; on the other hand, say we could consistently contract the minimal genus one subcurve: by smoothing the node $q$ and doing so, we would get a family of $3$-fold elliptic points specialising to the tacnode, which can be excluded by studying the miniversal deformation of the latter. This problem is studied in \cite[\S 4.1]{SMY2}. \end{rmk}
...
This concludes the proof of Proposition \ref{prop:1-stab}. I will record here a useful fact for the study of maps to $\PP^N$. Observe that, if $\nu\colon\PP^1\to \overline{C}$ is the normalisation of a Gorenstein genus one singularity, then $\nu^*\colon \Pic^d(\overline{C})\to \{\OO_{\PP^1}(d)\}$ has kernel isomorphic to $\mathbb G_a$; on the other hand the pullback of sections of a line bundle $\mathcal L$ on $\overline{C}$ gives a hyperplane in $H^0(\PP^1,\OO_{\PP^1}(d))$ by Riemann-Roch.
